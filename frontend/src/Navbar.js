...
function Navbar() {
  ...
  const { userDropdownStatus, setUserDropdownStatus, ref2 } =
    useOutsideAlerter2(false);
  const { adminDropdownStatus, setAdminDropdownStatus, ref3 } =
    useOutsideAlerter3(false);
  ...
  return (
    <div className="navbar__outer">
      <div className="navbar">
        ...
        <div
          className={click ? "navbar__option active" : "navbar__option"}
          ref={ref1}
        >
          {loadingCategories ? (
            <Loadingmsg />
            ...
            categories.map((c) => (
              <Link
                className="navbar__link navbar__category"
        ...
        <div className="navbar__logincart">
          {userInfo ? (
            <div className="dropdown" onClick={handleUserDropdown} ref={ref2}>
              <Link className="click__button" to="#">
                {acronym}
              </Link>
              <ul
                className={
                  userDropdownStatus
                    ? "dropdown__content active navbar__ul"
                    : "dropdown__content"
                }
              >
            ...
            <Link className="click__button" to="/signin">
              Login
            </Link>
          )}
          {userInfo && userInfo.isAdmin && (
            <div className="dropdown" onClick={handleAdminDropdown} ref={ref3}>
              <Link className="click__button" to="#admin">
                Admin
              </Link>
              <ul
                className={
